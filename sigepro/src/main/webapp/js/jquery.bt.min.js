...
;(function(jQuery) { 
    ...
    return this.each(function(index) {
      ...
      if (opts.killTitle) {
        jQuery(this).find('[title]').andSelf().each(function() {
          if (!jQuery(this).prop('bt-xTitle')) {
            jQuery(this).prop('bt-xTitle', jQuery(this).prop('title')).prop('title', '');
          }
        });
      }    
      ...
      this.btOn = function () {
		...
		this.changeText = function(message){
			//alert(message);
		jQuery('.bt-content', [jQuerybox[0]]).html(message);
		};
		
		this.getTextMessage = function(){
		var text = jQuery('.bt-content', [jQuerybox[0]]).html();
		return text;
		};
  ...
  */
  jQuery.fn.btOuterWidth = function(margin) {
      ...
      function num(elem, prop) {
    	  //TODO problems with new version of JQuery curCss for css
          return elem[0] && parseInt(jQuery.css(elem[0], prop, true), 10) || 0;
      };
  ...
  jQuery.fn.btOn = function() {
    return this.each(function(index){
	 ...
	 jQuery.fn.newMessage = function(message) {
    return this.each(function(index){
      if (jQuery.isFunction(this.newMessage)) {
        this.newMessage(message);
      }
    });
	};
  
    jQuery.fn.changeText = function(message) {
    return this.each(function(index){
      if (jQuery.isFunction(this.changeText)) {
        this.changeText(message);
      }
    });
  }; 
  
    jQuery.fn.getTextMessage = function(message) {
    var text="";
	this.each(function(index){
      if (jQuery.isFunction(this.getTextMessage)) {
        var aux = this.getTextMessage();
		text+=aux;
      }
    });
	return text;
