...
jQuery(function() {
		cleanScreenSearch();
		 ...
		 jQuery("#searchButton").click(function(){
			 var nameClient = jQuery("#nombreSearch").val();
			 var idcliente = "";
			 ...
			 var nroCompra = jQuery("#nroCompra").val();
		     jQuery("#listOCTable").jqGrid('setGridParam',{
		    	     url:"<c:url value="/searchListOC.htm" />?clientId="+idcliente+"&fechaDesde="+fechaDesde+"&fechaHasta="+fechaHasta+"&nroCompra="+nroCompra,page:1}
		      ).trigger('reloadGrid');
		 });
		 
		 jQuery("#clearSearchButton").click(function(){
			 cleanScreenSearch();
			 jQuery("#cancelEditButton").trigger("click");
		 });
	 ...
	 jQuery("#listOCTable").jqGrid({
				 ...
				 width : 950,
				 height : 200,
				 colNames : ['','','Nombre cliente','Nro Compra',  'Fecha OC','Fecha Entrega', 'Fecha<br>Ult. edición',
					...
					},
					{
						...
						name : 'fechaUltEdicion',
						index : 'fechaUltEdicion',
						align: 'center',
						width : 80
					}, {
						name : 'preciototal',
					...
					},
					{
						...
						formatter:function(){
							return "<div style='margin-left: 8px;'>"+
									...
									'<div onmouseout="jQuery(this).removeClass(\'ui-state-hover\')" onmouseover="jQuery(this).addClass(\'ui-state-hover\');"  class="ui-pg-div ui-inline-edit" style="float: left; cursor: pointer; display: block;margin-left:10px" title="Generar reporte"><span op="report" class="ui-icon ui-icon-document"></span></div>'+											
							       "</div>";
							},
						width : 60
					}
...
// 				  gridview: true,
				  grouping: true, 
				  groupingView : { groupField : ['nombreCliente'], 
...
// 	        orgCollapseNode.call(this, node);
	...
	//Cambio el alto de la fila del texto de los headers de las columnas
	jQuery("#listOCTable_fechaUltEdicion span:eq(0)").css({margin:"6px 0px"});
	jQuery("#jqgh_listOCTable_fechaUltEdicion").css({height:"30px"});
...
//EDIT OC SECTION OF JS!
     var lastsel; 
		...
		jQuery("#ordenCompraTable").jqGrid('navGrid','#ordenCompraTableGrid',{
		 ...
		 jQuery("#cancelEditButton").click(function(){
  	...
  	<div class="editOCForm" style="display: none;">	
        ...
        <form id="editOrdenCompraForm" action="" >
			...
			<div class="span-4 last"> 
				<fieldset>
            		<legend>Fecha</legend>
   						<input disabled="disabled" type="text" class="text2"  id="fechaOC" name="fechaOC" style="width: 80px" />
				</fieldset>
			</div>
			...
			<div class="span-24 last"> 
				<fieldset>           		
		              ...
		              <input type="button" id="cancelEditButton" value="Cancelar" />	
					</span>
				</fieldset>
			</div>
        </form>
